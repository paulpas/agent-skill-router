...
metadata:
  version: "1.0.0"
  domain: coding
  triggers: event-driven architecture, pub sub, message broker, Kafka, RabbitMQ, event streaming, dead letter queue, outbox pattern, idempotent consumer, exactly once semantics, schema registry, CloudEvents
  archetypes:
    - tactical
    - generation
  anti_triggers:
