...
description: Implements event sourcing pattern (event store, aggregate roots, projections, snapshots, event replay) to maintain complete audit trail and reconstruct state from immutable event history.
license: MIT
compatibility: opencode
metadata:
  version: "1.0.0"
  domain: coding
  triggers: event sourcing, event store, aggregate root, event replay, projections, snapshots, how do i track all changes, immutable audit trail, state reconstruction from events
  role: implementation
