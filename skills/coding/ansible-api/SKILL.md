---
name: ansible-api
description: Integrates with Ansible via ansible-runner, the AWX/Tower API, and the Ansible Python API to manage playbooks, inventory, job templates, collections, and automation workflows.
license: MIT
compatibility: opencode
metadata:
  version: "1.0.0"
  domain: coding
  triggers: ansible api, ansible-runner, ansible tower, awx api, ansible playbook, ansible inventory, ansible collections, automation controller
  role: implementation
...
- Embedding Ansible playbook execution directly into Python applications or CI/CD pipelines
- Automating job template creation, credential management, and inventory updates in AWX/Ansible Automation Controller
- Building self-service automation portals that trigger Ansible runs on demand
- Implementing dynamic inventory scripts that query cloud APIs or CMDBs
- Developing custom Ansible modules, filter plugins, or callback plugins
- Running ad-hoc Ansible commands programmatically across hosts
...
- Declarative infrastructure provisioning (use `coding-terraform-sdk` or `coding-pulumi`)
- Docker container orchestration (use `coding-docker-api` or `coding-kubernetes-api`)
- Simple shell command execution on remote hosts (use `fabric` or `paramiko` directly)

---

## Core Workflow

1. **Prepare Playbook and Inventory** — Create or load the Ansible playbook YAML and inventory file (INI, YAML, or dynamic). **Checkpoint:** Validate the playbook syntax locally with `ansible-playbook --syntax-check`.

2. **Execute via ansible-runner** — Call `ansible_runner.run(private_data_dir, playbook, inventory)`. The runner manages artifacts, event callbacks, and result collection. **Checkpoint:** Check `runner.status` is `successful` and `runner.rc == 0`.

3. **Process Results** — Iterate over `runner.events` for per-host results. Use `runner.get_fact_cache(host)` for gathered facts. **Checkpoint:** Verify expected hosts are in `runner.stats.processed` and none in `runner.stats.failures`.

4. **Manage AWX/Automation Controller** — Use the AWX REST API or `awxkit` to create job templates, launch jobs, and monitor their status through the state machine (pending → waiting → running → successful). **Checkpoint:** Verify job template `extra_vars` are properly JSON-encoded.

5. **Handle Errors** — Inspect `runner.stats.failures`, `runner.stats.dark` (unreachable hosts), and individual event data for task-level errors. **Checkpoint:** Distinguish between unreachable hosts (network issues) and failed tasks (playbook logic).
...
```python
#!/usr/bin/env python3
"""Ansible dynamic inventory script for cloud instances.

Returns inventory in the JSON format expected by Ansible:
{
    "group_name": {
        "hosts": ["host1", "host2"],
        "vars": {"key": "value"}
...
def get_cloud_instances() -> dict:
    """Query a cloud API and return Ansible-compatible inventory.

    This example simulates querying EC2 instances. Replace with
    real cloud API calls (boto3, google-cloud-sdk, etc.).

    Returns:
        Dict in Ansible inventory JSON format.
    """
    # Simulated cloud API response
    instances = [
        {"name": "web-01", "ip": "10.0.1.10", "group": "web", "region": "us-east-1"},
        {"name": "web-02", "ip": "10.0.1.11", "group": "web", "region": "us-east-1"},
        {"name": "db-01", "ip": "10.0.2.10", "group": "db", "region": "us-east-1"},
    ]
...
if __name__ == "__main__":
    # Ansible calls with --list for full inventory
    # or --host <hostname> for a single host
    if os.environ.get("ANSIBLE_INVENTORY_ACTION") == "--host":
        hostname = os.environ.get("ANSIBLE_HOST")
        inventory = get_cloud_instances()
        print(json.dumps(inventory.get("_meta", {}).get("hostvars", {}).get(hostname, {})))
    else:
        print(json.dumps(get_cloud_instances()))
```
...
- [ansible-runner Python Interface Documentation](https://ansible-runner.readthedocs.io/en/latest/python_interface.html)
- [Ansible Python API (Internal)](https://docs.ansible.com/ansible/latest/dev_guide/developing_api.html)
- [AWX/Ansible Automation Controller API Reference](https://docs.ansible.com/automation-controller/latest/html/controllerapi/index.html)
- [AWX Project GitHub](https://github.com/ansible/awx)
- [Ansible Dynamic Inventory Documentation](https://docs.ansible.com/ansible/latest/dev_guide/developing_inventory.html)
- [Ansible Collection Development Guide](https://docs.ansible.com/ansible/latest/dev_guide/developing_collections.html)
- [Ansible Runner GitHub Repository](https://github.com/ansible/ansible-runner)

## Related Skills

| Skill | Purpose |
|---|---|
| `coding-terraform-sdk` | Terraform for provisioning infrastructure that Ansible configures |
| `coding-kubernetes-api` | Kubernetes management via Ansible k8s module |
| `coding-docker-api` | Docker container management complementary to Ansible |
