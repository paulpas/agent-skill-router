...
description: Evaluates and selects the optimal AI agent framework (LangChain, CrewAI, LlamaIndex, DSPy, Microsoft Agent Framework) for a project based on capability requirements, production constraints, and team expertise.
license: MIT
compatibility: opencode
metadata:
  version: "1.0.0"
  domain: agent
  triggers: ai framework selection, which ai framework to use, langchain vs crewai, choose ai agent framework, framework comparison, build custom vs use framework, AI agent tooling, how do i pick an ai framework, LLM framework evaluation
  role: orchestration
  scope: orchestration
  output-format: analysis
  content-types: [guidance, examples, do-dont, diagrams]
  related-skills: framework-selection, framework-orchestration-routing, orchestration-frameworks, agent-architecture-patterns
---
