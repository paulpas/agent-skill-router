...
main() {
    ...
    prompt+="\nPhase 1 (Structural Checks via validate_skill_yaml.py):"
    prompt+="\n  • YAML frontmatter must parse correctly with yaml.safe_load()"
    prompt+="\n  • name: field in frontmatter MUST exactly match the directory name (kebab-case)"
    prompt+="\n  • version: field must be quoted — use \"1.0.0\" not 1.0.0"
    prompt+="\n  • Frontmatter delimiters must be exactly --- (not ------ or more dashes)"
    prompt+="\n  • name: must appear on line 2, immediately after the opening ---"
    prompt+="\n  • metadata block MUST include all required fields: triggers, domain, role, scope, output-format"
    prompt+="\n  • triggers must have 3-8 comma-separated terms (no more, no less)"
    prompt+="\n  • If role is 'implementation' or 'review', the skill MUST have a ## Constraints section with ### MUST DO and ### MUST NOT DO subsections"
    ...
    prompt+="\nAfter generating the SKILL.md file, run: ./scripts/validate_skill.sh <path-to-created-file>"
    prompt+="\nIf validation FAILS, fix the issues immediately and re-run until it PASSES before finishing."
    prompt+="\nDo NOT commit or finish until the skill passes all phases of validation."
